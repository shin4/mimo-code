...
afterEach(() => {
  delete process.env.MIMO_PLUGIN_META_FILE
})
...
describe("plugin.meta", () => {
  test("tracks file plugin loads and changes", async () => {
    ...
    process.env.MIMO_PLUGIN_META_FILE = path.join(tmp.path, "state", "plugin-meta.json")
    const file = process.env.MIMO_PLUGIN_META_FILE!
